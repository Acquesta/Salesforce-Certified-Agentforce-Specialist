...
Questions live in `src/data/questions/<section>.ts` and follow the `Question` type in `src/types/quiz.ts`. Every option needs an `explanation`. Multiple-select questions list more than one id in `correct`; the UI shows "Choose N answers", so do not repeat it in the prompt. `npm run test` validates the bank.
